...
Client.prototype.addTorrent = function(file, state) {
  var self = this;
  var torrent = new Torrent(self.clientId, self.port, file, self.downloadPath, state);
...
Client.prototype.handleConnection = function(stream) {
  ...
  stream.on('error', function(e) {
    LOGGER.debug('Peer [' + peer.getIdentifier() + '] connection error');

    peer.disconnect(e.message);
  });
};
