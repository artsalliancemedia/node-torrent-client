...
var Piece = function(index, offset, length, hash, skipValidation, files, callback) {
  EventEmitter.call(this);

  var self = this;

  self.complete = new BitField(Math.ceil(length / Piece.CHUNK_LENGTH));
  self.files = [];
  self.hash = hash;
  self.index = index;
  self.length = length;
  ...
  self.setMaxListeners(0);//self.requested.length);
  ...
  for (var i = 0; i < files.length; i++) {
    var file = files[i];
    var match = file.contains(self.offset, self.length);
    if (match === File.FULL || (match === File.PARTIAL && lastMatch === File.PARTIAL)) {
