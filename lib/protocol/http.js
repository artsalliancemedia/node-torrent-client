...
HTTP.maxQueueLength = 5000;

// use async to serialize tracker announce http requests
// don't want to make more than min(maxSockets,20) concurrent requests
HTTP.announceQueue = async.queue(function(params, callback) {
  var instance = params.instance;

  // instance will call _complete once finished parsing the response
  instance._complete = function(trackerInfo, err) {
    // async callback - need to let async know we are done here
    callback();
    // - call back with result
    if (instance.callback) {
      instance.callback(trackerInfo, err);
    }
  };

  instance._makeRequest(params.compact);
}, Math.min(http.globalAgent.maxSockets, 20));
...
HTTP.prototype = {
  ...
  handle: function(tracker, data, compact, event, callback) {
    ...
    if (HTTP.announceQueue.length()>=HTTP.maxQueueLength) {
      process.nextTick(function() {
        callback(null, new Error('Too many announce requests pending, try later!'))
      })
    }
    else {
      HTTP.announceQueue.push({instance:this, compact:compact})
    }
  },
