...
var MAX_REQUESTS = 10;
var MAX_FLUSH_QUEUE_LENGTH = 1024; // pieces
...
// use async to serialize diskwrites
Peer.flushQueue = async.queue(
  flushWorker, // worker function
  5 // max concurrency level
);

function flushWorker(params, callback) {
  var self = params.peer;
    ...
    if (err) {
      if (err.code===Piece.ERR_FILEBUSY) {
        ...
        process.nextTick(function() {
          Peer.flushQueue.push({peer:self,piece:params.piece});
        })
      }
      else {
...
function onPieceComplete(piece) {
  ...
  Peer.flushQueue.push({peer:self,piece:piece})
...
Peer.prototype.requestPiece = function(piece) {
  ...
  var writeBufferFull = Peer.flushQueue.length() >= MAX_FLUSH_QUEUE_LENGTH;
