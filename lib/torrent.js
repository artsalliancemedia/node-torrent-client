...
var Torrent = function(clientId, port, file, downloadPath, state) {
  ...
  self.status = Torrent.STATUS_LOADING;
  ...
  fs.readFile(file, 'binary', function(err, data) {
    if (err) {
      ...
      parse(self, data, state, function() {
        LOGGER.info('Torrent initialized');
      });
    }
  });
};
  ...
  (function create() {
    if (index === numPieces) {
      ...
      self.pieces[index] =
        new Piece(
          index, // piece index
          index * self.pieceLength, // piece offset
          pieceLength, // piece length
          hash, // piece hash from the torrent info
          self.hasState, // skip piece validation if content has state
          self.files, // array of files this piece contains data for
          function(piece) { // callled when piece has finished initialized
            if (self.hasState) {
              piece.isComplete = self.bitfield.isSet(piece.index);
            }
            if (piece.isComplete) {
...
function parse(self, data, state, cb) {
  ...
  try {
    ...
    if (state) {
      if (state.isComplete) {
        for (var idx=0;idx<self.bitfield.length;idx++) {
          self.bitfield.set(idx);
        }
        self.hasState = true;
      }
      else if (state.completedPieces) {
        for (var idx=0;idx<state.completedPieces.length;idx++) {
          self.bitfield.set(state.completedPieces[idx]);
        }
        self.hasState = true;
      }
    }
